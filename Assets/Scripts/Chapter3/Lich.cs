...
public class Lich : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Fire_SKill();
        if (StunOff == true && FreezeOff == true)
        {
            Reload_Skill();
        }

        if (anim.GetCurrentAnimatorStateInfo(0).IsName("LichSkill") &&
    ...
    void SummonSkelleton()
    {
        int rand_num = Random.Range(0, 3);
        GameObject skelleton = Instantiate(Skelleton[rand_num], transform.position, Quaternion.identity);
        Monster monster_skelleton = skelleton.GetComponent<Monster>();
        monster_skelleton.MovePoint = MovePoint;
    }
